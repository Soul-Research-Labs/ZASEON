# Intent Settlement Guide

> Tachyon-derived intent-based architecture for Soul Protocol

## Overview

Soul Protocol's intent settlement suite enables users to express **what** they want
(cross-chain state transfer) without specifying **how** it happens. Solvers compete
to fulfill intents, providing instant settlement with ZK privacy guarantees.
...
┌─────────────────────────────────────────────────────────────┐
│                    SoulProtocolHub                           │
│  Component 20: IntentSettlementLayer  (CORE)                │
│  Component 21: InstantSettlementGuarantee (CORE)            │
│  Component 22: DynamicRoutingOrchestrator (INFRASTRUCTURE)  │
└───────────┬──────────────────┬──────────────────┬───────────┘
            │                  │                  │
┌───────────▼──────┐ ┌────────▼─────────┐ ┌──────▼──────────┐
│ IntentSettlement │ │ InstantSettlement │ │ DynamicRouting  │
...
```typescript
import { createIntentSettlementClient, IntentStatus } from "@soul/sdk";
...
This deploys IntentSettlementLayer, InstantSettlementGuarantee, and DynamicRoutingOrchestrator
...
```bash
# Integration tests
forge test --match-path "test/integration/IntentSettlementE2E*" -vv
forge test --match-path "test/integration/CompliancePrivacyE2E*" -vv

# Invariant/fuzz tests
forge test --match-path "test/invariant/SettlementInvariants*" -vv --fuzz-runs 10000
...
6. **isFinalized vs canFinalize**: Settlement guarantee checks actual finalization state, not just eligibility
