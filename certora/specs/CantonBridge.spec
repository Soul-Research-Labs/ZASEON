/**
 * Certora Verification Spec: CantonBridgeAdapter
 *
 * Verifies critical invariants for the Soul Protocol <-> Canton Network bridge:
 * - Nonce monotonicity
 ...
 * - Pause mechanism
 *
 * Canton Network-specific properties:
 * - Chain ID = 510 (canton-global-1 EVM mapping)
 * - 1 CANTON = 1e6 microcanton (6 decimal precision)
 * - Default round confirmations = 5 (~10s synchronizer finality)
 * - 0.05% bridge fee (5 BPS)
 * - Uses mediator attestations (not validator signatures)
 * - Synchronizer rounds (not blocks)
 */

using CantonBridgeAdapter as bridge;
...
/// @title Canton chain ID is always 510
invariant cantonChainIdConstant()
    bridge.CANTON_CHAIN_ID() == 510;

/// @title Microcanton per CANTON is always 1e6
invariant microcantonPerCantonConstant()
    bridge.MICROCANTON_PER_CANTON() == 1000000;

/// @title Min deposit is always 0.1 CANTON (1e5 microcanton)
invariant minDepositConstant()
    bridge.MIN_DEPOSIT_MICROCANTON() == 100000;

/// @title Bridge fee BPS is always 5 (0.05%)
invariant bridgeFeeBpsConstant()
    bridge.BRIDGE_FEE_BPS() == 5;

/// @title Withdrawal refund delay is always 72 hours
invariant withdrawalRefundDelayConstant()
    bridge.WITHDRAWAL_REFUND_DELAY() == 259200;

/// @title Min escrow timelock is always 2 hours
invariant minEscrowTimelockConstant()
    bridge.MIN_ESCROW_TIMELOCK() == 7200;

/// @title Max escrow timelock is always 60 days
invariant maxEscrowTimelockConstant()
    bridge.MAX_ESCROW_TIMELOCK() == 5184000;

/// @title Default round confirmations is always 5
invariant defaultRoundConfirmationsConstant()
    bridge.DEFAULT_ROUND_CONFIRMATIONS() == 5;

/*//////////////////////////////////////////////////////////////
            RULE: REPLAY PROTECTION IS PERMANENT
//////////////////////////////////////////////////////////////*/

/// @title Once a Canton tx hash is used, it stays used forever
rule replayProtectionPermanence(bytes32 txHash, method f) {
    bool usedBefore = bridge.usedCantonTxHashes(txHash);
    ...
    bool usedAfter = bridge.usedCantonTxHashes(txHash);
...
rule onlyOperatorCanCompleteDeposit(env e) {
    ...
    bridge.completeCANTONDeposit@withrevert(e, depositId);
...
rule pauseBlocksDeposits(env e) {
    ...
    uint256 amount; uint256 roundNum;
    ICantonBridgeAdapter.CantonMerkleProof proof;
    ICantonBridgeAdapter.MediatorAttestation[] attestations;

    bridge.initiateCANTONDeposit@withrevert(
        e, txHash, sender, recipient, amount, roundNum, proof, attestations
    );
...
/*//////////////////////////////////////////////////////////////
            RULE: ROUND NUMBER MONOTONICITY
//////////////////////////////////////////////////////////////*/

/// @title Latest round number never decreases
rule latestRoundNeverDecreases(method f) {
    uint256 roundBefore = bridge.latestRoundNumber();

    env e;
    calldataarg args;
    f(e, args);

    uint256 roundAfter = bridge.latestRoundNumber();
    assert roundAfter >= roundBefore, "Latest round number decreased";
}
