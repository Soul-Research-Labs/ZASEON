/**
 * Certora Verification Spec: BNBBridgeAdapter
 *
 * Verifies critical invariants for the Soul Protocol <-> BNB Chain bridge:
 * - Nonce monotonicity
...
using BNBBridgeAdapter as bridge;
...
/// @title BSC chain ID is always 56
invariant bscChainIdConstant()
    bridge.BSC_CHAIN_ID() == 56;

/// @title Wei per BNB is always 1e18
invariant weiPerBnbConstant()
    bridge.WEI_PER_BNB() == 1000000000000000000;

/// @title Min deposit is always 0.01 BNB (1e16 wei)
invariant minDepositConstant()
    bridge.MIN_DEPOSIT_WEI() == 10000000000000000;

/// @title Bridge fee BPS is always 25 (0.25%)
invariant bridgeFeeBpsConstant()
    bridge.BRIDGE_FEE_BPS() == 25;
...
/// @title Default block confirmations is always 15
invariant defaultBlockConfirmationsConstant()
    bridge.DEFAULT_BLOCK_CONFIRMATIONS() == 15;

/*//////////////////////////////////////////////////////////////
            RULE: REPLAY PROTECTION IS PERMANENT
//////////////////////////////////////////////////////////////*/

/// @title Once a BSC tx hash is used, it stays used forever
rule replayProtectionPermanence(bytes32 txHash, method f) {
    bool usedBefore = bridge.usedBSCTxHashes(txHash);
    ...
    bool usedAfter = bridge.usedBSCTxHashes(txHash);
...
rule onlyOperatorCanCompleteDeposit(env e) {
    ...
    bridge.completeBNBDeposit@withrevert(e, depositId);
...
rule pauseBlocksDeposits(env e) {
    ...
    IBNBBridgeAdapter.BSCMerkleProof proof;
    IBNBBridgeAdapter.ValidatorAttestation[] attestations;

    bridge.initiateBNBDeposit@withrevert(
