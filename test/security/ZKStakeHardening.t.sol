...
contract ZKStakeHardeningTest is Test {
    ...
    function createLockAndOptimisticUnlock()
        public
        returns (bytes32, ZKBoundStateLocks.UnlockProof memory)
    {
        vm.startPrank(alice);
    ...
    function test_Harden_ChallengeStakeBoundary() public {
        (
            bytes32 lockId,
            ZKBoundStateLocks.UnlockProof memory proof
        ) = createLockAndOptimisticUnlock();
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "InsufficientChallengerStake(uint256,uint256)",
                MIN_CHALLENGER_STAKE,
                MIN_CHALLENGER_STAKE - 1
            )
        );
        locks.challengeOptimisticUnlock{value: MIN_CHALLENGER_STAKE - 1}(
            lockId,
            proof
        );
        ...
        locks.challengeOptimisticUnlock{value: MIN_CHALLENGER_STAKE}(
            lockId,
            proof
        );
    ...
    function test_Harden_FailedChallenge_CompensatesUnlocker() public {
        ...
        assertEq(
            alice.balance,
            aliceBalance + stake,
            "Unlocker should receive challenger stake"
        );
    }
    ...
    function test_Harden_SuccessfulChallenge_RewardsChallenger() public {
        ...
        assertEq(
            bob.balance,
            bobBalance - stake + stake + bond,
            "Challenger should get stake back + bond"
        );
    }
}
