...
contract CrossChainPrivacyHubTest is Test {
    ...
    function test_setSoulProtocolHub() public {
        hub.setSoulProtocolHub(address(0x1));
        assertEq(hub.soulProtocolHub(), address(0x1));
    }

    function test_setSoulProtocolHub_revertOnZero() public {
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setSoulProtocolHub(address(0));
    }

    function test_setMLSAGSignatures() public {
        hub.setMLSAGSignatures(address(0x2));
        assertEq(hub.mlsagSignatures(), address(0x2));
    }

    function test_setRingConfidentialTransactions() public {
        hub.setRingConfidentialTransactions(address(0x3));
        assertEq(hub.ringConfidentialTransactions(), address(0x3));
    }

    function test_setMixnetNodeRegistry() public {
        hub.setMixnetNodeRegistry(address(0x4));
        assertEq(hub.mixnetNodeRegistry(), address(0x4));
    }

    function test_setDecoyTrafficGenerator() public {
        hub.setDecoyTrafficGenerator(address(0x5));
        assertEq(hub.decoyTrafficGenerator(), address(0x5));
    }

    function test_setGasNormalizer() public {
        hub.setGasNormalizer(address(0x6));
        assertEq(hub.gasNormalizer(), address(0x6));
    }

    function test_setStealthRegistry() public {
        hub.setStealthRegistry(address(0x7));
        assertEq(hub.stealthRegistry(), address(0x7));
    }

    function test_setPrivateRelayerNetwork() public {
        hub.setPrivateRelayerNetwork(address(0x8));
        assertEq(hub.privateRelayerNetwork(), address(0x8));
    }

    function test_setComplianceModule() public {
        hub.setComplianceModule(address(0x9));
        assertEq(hub.complianceModule(), address(0x9));
    }

    function test_allModuleSetters_revertOnZero() public {
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setMLSAGSignatures(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setRingConfidentialTransactions(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setMixnetNodeRegistry(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setDecoyTrafficGenerator(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setGasNormalizer(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setStealthRegistry(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setPrivateRelayerNetwork(address(0));
        vm.expectRevert(CrossChainPrivacyHub.ZeroAddress.selector);
        hub.setComplianceModule(address(0));
    }
    ...
    function test_initiatePrivateTransfer_basic() public {
        vm.prank(user);
        bytes32 reqId = hub.initiatePrivateTransfer{value: 1 ether}(
            DEST_CHAIN,
            keccak256("recipient"),
            1 ether,
    ...
    function test_initiatePrivateTransfer_sendsFee() public {
        ...
        vm.prank(user);
        hub.initiatePrivateTransfer{value: amount}(
            DEST_CHAIN,
            keccak256("r"),
            amount,
            CrossChainPrivacyHub.PrivacyLevel.BASIC,
    ...
    function test_completeTransfer_revertOnDoubleSpend() public {
        ...
        vm.prank(user);
        bytes32 reqId2 = hub.initiatePrivateTransfer{value: 1 ether}(
            DEST_CHAIN,
            keccak256("r2"),
            1 ether,
            CrossChainPrivacyHub.PrivacyLevel.BASIC,
    ...
    function test_dailyLimit_resetsAfterDay() public {
        ...
        vm.startPrank(user);
        hub.initiatePrivateTransfer{value: 5000 ether}(
            DEST_CHAIN,
            keccak256("r1"),
            5000 ether,
            CrossChainPrivacyHub.PrivacyLevel.BASIC,
