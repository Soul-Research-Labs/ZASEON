...
describe("PrivacyZoneManager", function () {
    ...
    describe("Zone Creation", function () {
        ...
        it("should create multiple zones with unique IDs", async function () {
            ...
            await manager.write.createZone([getDefaultZoneConfig("Zone_A")], { account: deployer.account });
            await manager.write.createZone([getDefaultZoneConfig("Zone_B")], { account: deployer.account });
            await manager.write.createZone([getDefaultZoneConfig("Zone_C")], { account: deployer.account });
    ...
    describe("Zone Status Management", function () {
        ...
        it("should update zone status", async function () {
            ...
            await manager.write.createZone([getDefaultZoneConfig()], { account: deployer.account });
            const activeIds = await manager.read.getActiveZoneIds();
            const zoneId = activeIds[0];

            // Pause the zone
            await manager.write.setZoneStatus([zoneId, 2], { account: deployer.account }); // Paused
        ...
        it("should remove zone from active list on shutdown", async function () {
            ...
            await manager.write.createZone([getDefaultZoneConfig("Zone1")], { account: deployer.account });
            await manager.write.createZone([getDefaultZoneConfig("Zone2")], { account: deployer.account });

            let activeIds = await manager.read.getActiveZoneIds();
            expect(activeIds.length).to.equal(2);

            // Shutdown first zone
            await manager.write.setZoneStatus([activeIds[0], 4], { account: deployer.account }); // Shutdown
    ...
    describe("Cross-Zone Migration", function () {
        ...
        it("should migrate state between zones in test mode", async function () {
            ...
            // Create two zones
            await manager.write.createZone([getDefaultZoneConfig("Source")], { account: deployer.account });
            await manager.write.createZone([getDefaultZoneConfig("Dest")], { account: deployer.account });
            ...
            expect(await manager.read.isNullifierSpent([sourceZone, nullifier])).to.be.true;

            // Verify nullifier NOT spent on dest (different scope)
            expect(await manager.read.isNullifierSpent([destZone, nullifier])).to.be.false;

            // Verify new commitment exists on dest
            expect(await manager.read.zoneCommitments([destZone, newCommitment])).to.be.true;
        ...
        it("should reject migration when zone has migration disabled", async function () {
            ...
            await manager.write.createZone([noMigrationConfig], { account: deployer.account });

            const activeIds = await manager.read.getActiveZoneIds();

            try {
                await manager.write.migrateState(
                    [activeIds[0], activeIds[1], generateCommitment(1n), generateCommitment(2n), "0x"],
                    { account: user1.account }
                );
                expect.fail("Should have reverted");
            } catch (e: any) {
    ...
    describe("Zone Isolation", function () {
        ...
        it("should maintain separate Merkle roots per zone", async function () {
            ...
            await manager.write.createZone([getDefaultZoneConfig("ZoneA")], { account: deployer.account });
            await manager.write.createZone([getDefaultZoneConfig("ZoneB")], { account: deployer.account });
        ...
        it("should have independent nullifier registries", async function () {
            ...
            expect(await manager.read.isNullifierSpent([activeIds[0], nullifier])).to.be.true;
            expect(await manager.read.isNullifierSpent([activeIds[1], nullifier])).to.be.false;
        });
    });
    ...
    describe("Policy Management", function () {
        ...
        it("should update zone policy", async function () {
            ...
            await manager.write.setZonePolicy([zoneId, newPolicy], { account: deployer.account });
