...
import "../../contracts/governance/SoulToken.sol";
...
contract SoulTokenTest is Test {
    ...
    function setUp() public {
        token = new SoulToken(admin, treasury, INITIAL_MINT);
    ...
    function test_Name() public view {
        assertEq(token.name(), "Soul Protocol");
    }
    ...
    function test_Constructor_ZeroAdmin_Reverts() public {
        ...
        new SoulToken(address(0), treasury, INITIAL_MINT);
    }

    function test_Constructor_ZeroRecipient_Reverts() public {
        vm.expectRevert(SoulToken.ZeroAddress.selector);
        new SoulToken(admin, address(0), INITIAL_MINT);
    }

    function test_Constructor_ZeroInitialMint() public {
        SoulToken t = new SoulToken(admin, treasury, 0);
    ...
    function test_Mint_ZeroAmount_Reverts() public {
        vm.prank(minter);
        vm.expectRevert(SoulToken.MintAmountZero.selector);
        token.mint(alice, 0);
    }
    ...
    function test_Burn_ZeroAmount_Reverts() public {
        vm.prank(alice);
        vm.expectRevert(SoulToken.BurnAmountZero.selector);
        token.burn(0);
    }
