...
contract VDFVerifierTest is Test {
    ...
    uint256 constant LARGE_MODULUS =
    ...
    function test_BeaconConsistencyAcrossBlocks() public {
        ...
        VDFVerifier.RandomnessBeacon memory b1 = vdf.getBeacon(
            uint64(block.chainid),
            1
        );
        VDFVerifier.RandomnessBeacon memory b2 = vdf.getBeacon(
            uint64(block.chainid),
            2
        );
        VDFVerifier.RandomnessBeacon memory b3 = vdf.getBeacon(
            uint64(block.chainid),
            3
        );
