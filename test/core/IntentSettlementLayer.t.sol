...
contract IntentSettlementLayerTest is Test {
    IntentSettlementLayer public layer;
    ...
    function setUp() public {
        ...
        layer = new IntentSettlementLayer(admin, address(verifier));
    ...
    function test_SubmitIntent() public {
        ...
        IIntentSettlementLayer.Intent memory intent = layer.getIntent(intentId);
        ...
        assertEq(
            uint(intent.status),
            uint(IIntentSettlementLayer.IntentStatus.PENDING)
        );
    ...
    function test_RevertOnSubmit_InvalidSourceChain() public {
        vm.prank(user1);
        vm.expectRevert(IIntentSettlementLayer.InvalidChainId.selector);
    ...
    function test_RevertOnSubmit_InsufficientFee() public {
        vm.prank(user1);
        vm.expectRevert(IIntentSettlementLayer.InsufficientFee.selector);
    ...
    function test_RevertOnSubmit_DeadlineTooSoon() public {
        vm.prank(user1);
        vm.expectRevert(IIntentSettlementLayer.InvalidDeadline.selector);
        layer.submitIntent{value: 1 ether}(
            SOURCE_CHAIN,
            DEST_CHAIN,
            COMMITMENT,
    ...
    function test_RevertOnSubmit_ZeroCommitment() public {
        vm.prank(user1);
        vm.expectRevert(IIntentSettlementLayer.InvalidAmount.selector);
        layer.submitIntent{value: 1 ether}(
            SOURCE_CHAIN,
            DEST_CHAIN,
            bytes32(0),
    ...
    function test_CancelIntent() public {
        ...
        assertEq(
            ...
            uint(IIntentSettlementLayer.IntentStatus.CANCELLED)
        );
    }
    ...
    function test_RevertOnCancel_NotUser() public {
        ...
        vm.prank(user2);
        vm.expectRevert(IIntentSettlementLayer.NotIntentUser.selector);
        layer.cancelIntent(intentId);
    }
    ...
    function test_RevertOnCancel_NotPending() public {
        ...
        vm.prank(user1);
        vm.expectRevert(IIntentSettlementLayer.IntentNotPending.selector);
        layer.cancelIntent(intentId);
    }
    ...
    function test_RegisterSolver() public {
        ...
        IIntentSettlementLayer.Solver memory s = layer.getSolver(solver1);
    ...
    function test_RevertOnRegister_InsufficientStake() public {
        vm.prank(solver1);
        vm.expectRevert(IIntentSettlementLayer.InsufficientStake.selector);
    ...
    function test_RevertOnRegister_AlreadyRegistered() public {
        ...
        vm.prank(solver1);
        vm.expectRevert(
            IIntentSettlementLayer.SolverAlreadyRegistered.selector
        );
        layer.registerSolver{value: 1 ether}();
    }
    ...
    function test_ClaimIntent() public {
        ...
        assertEq(
            uint(intent.status),
            uint(IIntentSettlementLayer.IntentStatus.CLAIMED)
        );
    }
    ...
    function test_RevertOnClaim_SolverNotActive() public {
        ...
        vm.prank(solver1);
        vm.expectRevert(IIntentSettlementLayer.SolverNotActive.selector);
        layer.claimIntent(intentId);
    }
    ...
    function test_RevertOnClaim_DeadlinePassed() public {
        ...
        vm.prank(solver1);
        vm.expectRevert(IIntentSettlementLayer.DeadlinePassed.selector);
        layer.claimIntent(intentId);
    }
    ...
    function test_FulfillIntent() public {
        ...
        assertEq(
            uint(intent.status),
            uint(IIntentSettlementLayer.IntentStatus.FULFILLED)
        );
    ...
    function test_RevertOnFulfill_NotAssignedSolver() public {
        ...
        vm.prank(solver2);
        vm.expectRevert(IIntentSettlementLayer.NotAssignedSolver.selector);
        layer.fulfillIntent(
            intentId,
            hex"1234",
            hex"5678",
    ...
    function test_RevertOnFulfill_InvalidProof() public {
        ...
        vm.prank(solver1);
        vm.expectRevert(IIntentSettlementLayer.InvalidProof.selector);
        layer.fulfillIntent(
            intentId,
            hex"1234",
            hex"5678",
    ...
    function test_FinalizeIntent() public {
        ...
        assertEq(
            uint(intent.status),
            uint(IIntentSettlementLayer.IntentStatus.FINALIZED)
        );
    ...
    function test_RevertOnFinalize_ChallengePeriodActive() public {
        ...
        vm.expectRevert(IIntentSettlementLayer.ChallengePeriodActive.selector);
        layer.finalizeIntent(intentId);
    }
    ...
    function test_RevertOnFinalize_NotFulfilled() public {
        ...
        vm.expectRevert(IIntentSettlementLayer.IntentNotFulfilled.selector);
        layer.finalizeIntent(intentId);
    }
    ...
    function test_DisputeIntent() public {
        ...
        assertEq(
            uint(intent.status),
            uint(IIntentSettlementLayer.IntentStatus.DISPUTED)
        );
    ...
    function test_ExpireIntent_Pending() public {
        ...
        assertEq(
            uint(layer.intentStatus(intentId)),
            uint(IIntentSettlementLayer.IntentStatus.EXPIRED)
        );
    }
    ...
    function test_ExpireIntent_Claimed_SlashesSolver() public {
        ...
        IIntentSettlementLayer.Solver memory sBefore = layer.getSolver(solver1);
        layer.expireIntent(intentId);

        IIntentSettlementLayer.Solver memory sAfter = layer.getSolver(solver1);
    ...
    function test_RevertOnSetVerifier_ZeroAddress() public {
        vm.prank(admin);
        vm.expectRevert(IIntentSettlementLayer.ZeroAddress.selector);
