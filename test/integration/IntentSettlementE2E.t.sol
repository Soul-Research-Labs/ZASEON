...
/**
 * @title IntentSettlementE2E
 * @notice End-to-end tests for the Tachyon-inspired intent settlement suite.
 * @dev Tests the full lifecycle: Intent → Settlement → Guarantee → Rewards → Routing.
 *      Validates that the three unwired Tachyon contracts (IntentSettlementLayer,
 *      InstantSettlementGuarantee, DynamicRoutingOrchestrator) work together correctly
 *      and integrate with the Hub.
 */
contract IntentSettlementE2E is Test {
    // =========================================================================
    // CONTRACTS
    // =========================================================================
    ...
    function setUp() public {
        ...
        bytes32 SETTLEMENT_ROLE = guarantee.SETTLEMENT_ROLE();
        guarantee.grantRole(SETTLEMENT_ROLE, admin);

        // Enable chain on IntentSettlementLayer
    ...
    function test_fullIntentLifecycle() public {
        ...
        IIntentSettlementLayer.Solver memory solverInfo = intentLayer.getSolver(
            solver
        );
        ...
        assertEq(
            ...
            uint256(IIntentSettlementLayer.IntentStatus.PENDING)
        );
        ...
        assertEq(
            uint256(intent.status),
            uint256(IIntentSettlementLayer.IntentStatus.CLAIMED)
        );
        ...
        assertEq(
            uint256(intent.status),
            uint256(IIntentSettlementLayer.IntentStatus.FULFILLED)
        );
        ...
        assertEq(
            uint256(intent.status),
            uint256(IIntentSettlementLayer.IntentStatus.FINALIZED)
        );
        assertTrue(
            intentLayer.isFinalized(intentId),
    ...
    // =========================================================================
    // INSTANT SETTLEMENT: Guarantee Lifecycle
    // =========================================================================

    function test_instantSettlementWithGuarantee() public {
        ...
        // Solver posts guarantee bond for instant settlement
        ...
        assertEq(
            uint256(g.status),
            uint256(IInstantSettlementGuarantee.GuaranteeStatus.ACTIVE)
        );
        ...
        assertEq(
            uint256(g.status),
            uint256(IInstantSettlementGuarantee.GuaranteeStatus.SETTLED)
        );
        assertTrue(
            solver.balance > solverBalBefore,
    ...
    function test_guaranteeClaimOnExpiredUnfinalizedIntent() public {
        ...
        assertEq(
            uint256(g.status),
            uint256(IInstantSettlementGuarantee.GuaranteeStatus.CLAIMED)
        );
        assertTrue(
    ...
    function test_dynamicRoutingBridgeRegistration() public {
        ...
        vm.prank(bridgeAdmin);
        router.registerBridge(mockBridge, supportedChains, 9000); // 90% security score

        IDynamicRoutingOrchestrator.BridgeMetrics memory metrics = router
            .getBridgeMetrics(mockBridge);
    ...
    function test_hubWiringWithIntentComponents() public {
        ...
        assertTrue(foundIntent, "IntentSettlementLayer should be wired");
        assertTrue(
            foundGuarantee,
            "InstantSettlementGuarantee should be wired"
        );
    ...
    function test_intentExpiry() public {
        ...
        assertEq(
            uint256(intent.status),
            uint256(IIntentSettlementLayer.IntentStatus.EXPIRED)
        );
    }
    ...
    function test_fullEndToEndWithAllComponents() public {
        ...
        // 4. Solver posts instant settlement guarantee
