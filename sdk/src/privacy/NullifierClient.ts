...
import { ViemReadonlyContract, DecodedEventArgs } from "../types/contracts";
...
const NULLIFIER_MANAGER_ABI = [
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: "sourceDomain", type: "uint256" },
      { name: "targetDomain", type: "uint256" },
    ],
  ...
  },
  {
    ...
    type: "function",
    stateMutability: "view",
    inputs: [{ name: "domain", type: "uint256" }],
    outputs: [{ type: "bool" }],
  },
  {
    name: "getSoulBinding",
  ...
  {
    name: "CrossDomainNullifierDerived",
    type: "event",
    inputs: [
      { name: "sourceNullifier", type: "bytes32", indexed: true },
      { name: "crossNullifier", type: "bytes32", indexed: true },
      { name: "sourceDomain", type: "uint256" },
      { name: "targetDomain", type: "uint256" },
    ],
  },
...
export class NullifierClient {
  ...
  async deriveCrossDomainNullifier(
    ...
    const crossDomainNullifier =
      await this.contract.read.deriveCrossDomainNullifier([
        sourceNullifier,
  ...
  async isNullifierConsumed(
    ...
    return await this.contract.read.isNullifierConsumed([
      nullifier,
      BigInt(domainChainId),
    ]);
  }
    ...
    const unwatch = this.publicClient.watchContractEvent({
      ...
      onLogs: (logs) => {
        for (const log of logs) {
          const args = (log as unknown as { args: DecodedEventArgs }).args;
          callback(
            args.sourceNullifier as Hex,
            args.crossNullifier as Hex,
            Number(args.sourceDomain),
            Number(args.targetDomain),
          );
        }
      },
    });
