...
/**
 * SDK client for the Intent Completion suite (Tachyon Learning #1).
 *
 * Covers IntentCompletionLayer (cross-chain intent completion with competitive solvers)
 * and InstantCompletionGuarantee (bonded proof delivery guarantees).
 *
 * @example
 * ```ts
...
/**
 * Factory function to create an IntentCompletionClient.
 */
export function createIntentCompletionClient(
  config: IntentCompletionConfig,
): IntentCompletionClient {
  return new IntentCompletionClient(config);
}
