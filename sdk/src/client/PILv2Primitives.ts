...
/**
 * Client for PILv2Orchestrator contract
 * Coordinates operations across all PIL v2 primitives
 */
export class PILv2OrchestratorClient {
  private contract: Contract;
  private signer: Signer | null;
...
/**
 * Factory for creating PIL v2 primitive clients
 */
export class PILv2ClientFactory {
  constructor(
    private config: PILv2Config,
  ...
  /**
   * Create PILv2Orchestrator client
   */
  orchestrator(): PILv2OrchestratorClient {
    if (!this.config.orchestrator) {
      throw new Error("Orchestrator address not configured");
    }
    return new PILv2OrchestratorClient(
