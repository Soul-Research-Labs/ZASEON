...
/**
 * Soul Protocol - Compliance Provider Client
 *
 * SDK client for the SoulComplianceV2 contract, enabling:
...
export class SoulComplianceProvider {
    ...
    async registerProvider(name?: string, schemaHash?: Hash): Promise<void> {
        ...
        const schema = schemaHash || ('0x' + Array.from(
            new TextEncoder().encode(`soul:compliance:${providerName}:v1`)
