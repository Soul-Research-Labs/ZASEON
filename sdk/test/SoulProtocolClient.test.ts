...
import {
  ...
  type LockInfo,
  type ProtocolStats,
} from "../src/client/SoulProtocolClient";
...
/**
 * SoulProtocolClient creates its own viem clients in the constructor,
...
describe("SoulProtocolClient", () => {
  ...
  // Config validation
  // ==================================================================
  describe("SoulProtocolConfig", () => {
    it("should accept minimal config", () => {
      const config: SoulProtocolConfig = {
  ...
  describe("factory functions", () => {
    it("createSoulClient should return SoulProtocolClient instance", () => {
      // This will fail if getAddresses returns null for the chainId,
      // so we test with Sepolia which we know is configured
      try {
        const client = createSoulClient({ rpcUrl: "https://rpc.sepolia.org" });
        expect(client).to.be.instanceOf(SoulProtocolClient);
      } catch {
    ...
    it("createReadOnlySoulClient should create client without wallet", () => {
      try {
        const client = createReadOnlySoulClient("https://rpc.sepolia.org");
        expect(client).to.be.instanceOf(SoulProtocolClient);
    ...
    it("createReadOnlySoulClient should accept chainId", () => {
      try {
