...
import {
  ...
  type CredentialData,
} from "../src/compliance/SoulComplianceProvider";
...
function patchClient(
  provider: SoulComplianceProvider,
  stubs: {
...
describe("SoulComplianceProvider", () => {
  describe("constructor", () => {
    it("should create an instance with valid config", () => {
      const provider = new SoulComplianceProvider(makeConfig());
      expect(provider).to.be.instanceOf(SoulComplianceProvider);
    });
  });
  ...
  describe("registerProvider", () => {
    ...
    it("should generate a schema hash when not provided", async () => {
      ...
      // Default schema: hex(soul:compliance:test-provider:v1), padded to 64 hex chars
