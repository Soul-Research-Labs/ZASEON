...
contract VDFVerifier is ReentrancyGuard, Ownable, Pausable {
    ...
    function computeInputHash(
        ...
        return
            keccak256(
                abi.encodePacked(input.seed, input.iterations, input.modulus)
            );
    }
    ...
    function verifyWesolowskiProof(
        ...
        if (
            ...
            input.iterations > MAX_ITERATIONS
        ) {
        ...
        bytes32 challengeHash = keccak256(
            abi.encodePacked(x, proof.y, input.iterations)
        );
    ...
    function getBeaconRandomness(
        ...
        return
            uint256(
                keccak256(abi.encodePacked(beacon.commitment, purpose, index))
            );
    }
    ...
    function selectRelayer(
        uint256 round
    ) external view returns (RelayerSelection memory) {
        ...
        uint256 randomValue = uint256(
            keccak256(abi.encodePacked(latestBeaconCommitment, round))
        );
