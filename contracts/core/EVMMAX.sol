...
contract EVMMAX is ReentrancyGuard, Ownable, Pausable {
    ...
    function verifyMontgomeryProof(
        MontgomeryProof calldata proof
        ...
        bytes32 computedHash = keccak256(
            abi.encodePacked(proof.a, proof.b, proof.result, proof.modulus)
        );
    ...
    function verifySIMDProof(
        SIMDProof calldata proof
    ) external nonReentrant whenNotPaused returns (bool) {
        if (!supportedModuli[proof.modulus]) revert InvalidModulus();
        if (verifiedProofs[proof.proofHash]) revert ProofAlreadyVerified();
    ...
    function verifyCurveAddProof(
        CurveAddProof calldata proof
    ) external nonReentrant whenNotPaused returns (bool) {
        if (verifiedProofs[proof.proofHash]) revert ProofAlreadyVerified();
        ...
        (uint256 resultX, uint256 resultY) = abi.decode(
            result,
            (uint256, uint256)
        );
    ...
    function verifyBatch(bytes32 batchId) external onlyOwner returns (bool) {
    ...
    function getBatchContext(
        bytes32 batchId
    ) external view returns (BatchContext memory) {
