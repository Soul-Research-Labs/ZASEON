...
/**
 * @title MockHyperBFTValidatorOracle
 * @notice Mock HyperBFT validator oracle for testing
 * @dev Simulates Hyperliquid's 4-validator BFT committee with attestation verification
 *
 * Hyperliquid uses HyperBFT (modified HotStuff) with:
 * - 4 active validators
 * - 2/3+1 supermajority = 3/4 required for consensus
 * - Near-instant BFT finality (~0.6s / 3 blocks)
 */
contract MockHyperBFTValidatorOracle {
     ...
     * @dev In production, this would verify the actual ECDSA signature against the HyperBFT committee
     ...
     * @dev Hyperliquid requires 2/3+1 = 3/4 validators for consensus
